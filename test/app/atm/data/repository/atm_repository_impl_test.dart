...
void main() {
  ...
  group('login', () {
    ...
    test(
        'should return Right(AppException) with spesific message if command argument is not consists of 3 words',
        () async {
      // Arrange
    ...
    test(
        'should return Right(AppException) with spesific message if username contains spaces or special characters',
        () async {
      // Arrange
    ...
    test(
        'should return Right(AppException) with spesific message if pin is not numeric',
        () async {
      // Arrange
    ...
    test(
        'should return Right(AppException) with spesific message if pin is not 6 digits',
        () async {
      // Arrange
    ...
    test(
        'should return Right(AppException) with spesific message if current customer is still logged in',
        () async {
      // Arrange
      const tCommand = tValidCommand;
      const tMessage = alreadyLoggedIn;
      final tAtmModel2 = tAtmModel.copyWith(activeCustomer: tActiveCustomer);
    ...
    test(
        'should return Right(AppException) with spesific message if customer is found but pin is not match',
        () async {
      // Arrange
